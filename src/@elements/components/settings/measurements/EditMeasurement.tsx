...
import { useGetMeasureById } from 'src/libs/settings/useGetMeasurements'
...
const schema = yup.object({
  ...
  unit_measure_type: yup.string().oneOf(['product_materials', 'service']).required(),
  status: yup.boolean().default(true)
})

const EditMeasurement = ({ setOpen, selected, refetch }: Props) => {
  ...
  const { data } = useGetMeasureById(selected?.id)
  useEffect(() => {
    if (data) {
      setValue('name', data.name)
      setValue('code', data.code)
      setValue('short_name', data.short_name)
      setValue('unit_measure_type', data.unit_measure_type)
  ...
  const onSubmit: SubmitHandler<FormTypes> = async data => {
    if (selected) {
      try {
        await axiosInstance.patch<TMeasurement>(`/unit-measures/${selected.id}/`, data)
    ...
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Controller
          ...
          control={control}
          render={({ field }) => (
            <RadioGroup row onChange={field.onChange}>
              <FormControlLabel
                checked={field.value === 'product_materials'}
                value='product_materials'
                control={<Radio />}
